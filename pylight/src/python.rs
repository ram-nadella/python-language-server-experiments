...
pub fn parse_python_files_sequential(
    ...
    // Check for duplicate paths in the files list
    {
        ...
        if duplicates > 0 {
            info!(
                "Found {} duplicate paths in the input files list",
                duplicates
            );
        }
    }
    ...
    // Initialize path registry with all paths first
    {
        ...
        info!(
            "Initialized path registry with {} paths (unique: {})",
            files.len(),
...
pub fn parse_python_files_parallel(
    ...
    let path_indices: std::collections::HashMap<PathBuf, usize>;
    {
        ...
        info!(
            "Pre-indexed {} paths in global PathRegistry (paths.len={})",
            files.len(),
            global_registry.paths.len()
        );
    }
    ...
    files.par_chunks(chunk_size).for_each(|chunk| {
        ...
        if local_syntax_errors > 0 {
            stats
                .syntax_errors
                .fetch_add(local_syntax_errors, Ordering::Relaxed);
        }
        if local_io_errors > 0 {
            stats
                .io_errors
                .fetch_add(local_io_errors, Ordering::Relaxed);
        }
        if local_other_errors > 0 {
            stats
                .other_errors
                .fetch_add(local_other_errors, Ordering::Relaxed);
        }
    });
...
fn process_file_with_path_idx(
    ...
    // Group related parameters into a context struct
    let context = SymbolCollectionContext {
        file_path_index,
        module: &module,
        fully_qualified_module: &fully_qualified_module,
    };
...
// Helper struct to group related parameters and reduce function arguments
struct SymbolCollectionContext<'a> {
    file_path_index: usize,
    module: &'a str,
    fully_qualified_module: &'a str,
}

// Recursive function to collect symbols with reduced parameter count
fn collect_symbols_recursive(
    node: tree_sitter::Node,
    source: &str,
    context: &SymbolCollectionContext,
        ...
        match current_node.kind() {
            "function_definition" => {
                if let Some(name_node) = current_node.child_by_field_name("name") {
                    ...
                    let symbol_type = if parent_context.is_empty() {
                        ...
                        .last()
                        .is_some_and(|p| matches!(p.symbol_type, crate::symbols::SymbolType::Class))
                    {
                    ...
                    let symbol_context = crate::symbols::SymbolContext {
                        file_path_index: context.file_path_index,
                        line_number: current_node.start_position().row + 1,
                        module: context.module.to_string(),
                        fully_qualified_module: context.fully_qualified_module.to_string(),
                        symbol_type: symbol_type.clone(),
                        parent_context: parent_context.to_vec(),
                    };

                    function_symbols.insert(crate::symbols::Symbol {
                        name: name.clone(),
            ...
            "class_definition" => {
                if let Some(name_node) = current_node.child_by_field_name("name") {
                    ...
                    class_symbols.insert(crate::symbols::Symbol {
                        name: name.clone(),
                        context: symbol_context,
                    });
                            ...
                            "function_definition" | "class_definition" => {
                                ...
                                if child.kind() == "function_definition" {
                                    if let Some(name_node) = child.child_by_field_name("name") {
                                        ...
                                        let symbol_type = if parent_context.is_empty() {
                                            crate::symbols::SymbolType::Function
                                        } else if parent_context.last().is_some_and(|p| {
                                            matches!(
                                                p.symbol_type,
                                                crate::symbols::SymbolType::Class
                                            )
                                        ...
                                        let symbol_context = crate::symbols::SymbolContext {
                                            ...
                                            module: context.module.to_string(),
                                            fully_qualified_module: context
...
fn parse_file_and_update_stats(
    ...
    let (function_symbols, class_symbols) =
        process_file_with_path_idx(parser, path, base_dir, Some(file_path_index))?;
...
"#;
        ...
        let (functions, classes) =
            parse_python_file(&mut parser, &file_path, base_dir, &mut path_registry)?;
...
"#;
        ...
        // Make sure path registry has expected count
        {
            ...
            assert_eq!(
                path_registry.paths.len(),
                1,
                "Path registry should have 1 path"
            );
        }
    ...
    fn test_parse_invalid_python() -> Result<()> {
        ...
        create_test_python_file(
            &invalid_file,
            "def broken_function( print('missing closing paren')",
        )?;
...
"#;
        ...
        for f in &functions {
            println!(
                "  Function: {} at line {} in file index {}",
        ...
        for (key, count) in &function_counts {
            if *count > 1 {
                println!(
                    "WARNING: Duplicate entry found: {} appears {} times",
                    key, count
                );
            }
        }
        ...
        assert_eq!(
            path_registry.paths.len(),
            1,
            "PathRegistry should contain exactly 1 path"
        );
        assert_eq!(
            path_registry.paths[0], python_file,
            "PathRegistry should contain the correct file path"
        );

        // Check that the function at line 5 was correctly parsed
        let func_at_line_5 = functions
            .iter()
        ...
        assert_eq!(
            func_at_line_25.context.line_number, 25,
            "function_at_line_25 should be at line 25, got {}",
            func_at_line_25.context.line_number
        );

        assert_eq!(
            path_registry.get_path(func_at_line_25.context.file_path_index),
            &python_file,
        ...
        assert_eq!(
            class.context.line_number, 11,
            "ClassAtLine11 should be at line 11, got {}",
            class.context.line_number
        );

        assert_eq!(
            path_registry.get_path(class.context.file_path_index),
            &python_file,
        ...
        assert_eq!(
            function_at_line_5.context.line_number, 5,
            "Line number should be preserved in search results"
        );
        assert_eq!(
            path_registry.get_path(function_at_line_5.context.file_path_index),
            &python_file,
        ...
        assert_eq!(
            results.len(),
            1,
            "Should find exactly one match for ClassAtLine11"
        );

        let (symbol, _) = &results[0];
        assert_eq!(
            symbol.name, "ClassAtLine11",
            "Search should return the correct symbol"
        );
        assert_eq!(
            symbol.context.line_number, 11,
            "Line number should be preserved in search results"
        );
        assert_eq!(
            path_registry.get_path(symbol.context.file_path_index),
            &python_file,
            "File path should be preserved in search results"
        );
        ...
        assert!(
            ...
            "Path should be in the path_to_index map"
        );
        assert_eq!(
            stored_index.unwrap(),
            0,
            "Path index should be a valid index in the path registry"
        );
...
"#,
        ...
        assert_eq!(
            path_registry.paths.len(),
            2,
            "PathRegistry should contain exactly 2 paths"
        );
        ...
        let expected_classes = vec![("Class1_line6", 5, 0), ("Class2_line9", 9, 1)];
        ...
        for &(name, expected_line, file_idx) in &expected_classes {
            ...
            assert_eq!(
                class.context.line_number, expected_line,
                "{} should be at line {}, got {}",
                name, expected_line, class.context.line_number
            );

            assert_eq!(
                class.context.file_path_index, file_idx,
                "{} should have file_path_index {}, got {}",
            ...
            expected_functions.iter().chain(expected_classes.iter())
        {
            ...
            assert_eq!(
                results.len(),
                1,
                "Should find exactly one match for {}, found {}",
                name,
                results.len()
            );
            ...
            assert_eq!(
                symbol.context.line_number, expected_line,
                "Line number should be preserved in search results"
            );
            assert_eq!(
                symbol.context.file_path_index, file_idx,
                "File index should be preserved in search results"
            );
            assert_eq!(
                path_registry.get_path(symbol.context.file_path_index),
                &file_paths[file_idx],
...
"#;
        ...
        // Test parallel processing with duplicate paths
        {
            ...
            // Verify stats
            let (func_count, class_count, syntax_errors, io_errors, other_errors) =
                stats.get_counts();
            ...
            assert_eq!(
                path_registry.paths.len(),
                1,
                "Should have only one unique path"
            );
            assert_eq!(
                path_registry.path_to_index.len(),
                1,
                "Should have only one path in the index map"
            );
            ...
            let (results, _metrics) = crate::search::search_symbols(
                "function_one",
                &functions,
                &classes,
                &path_registry,
            ...
            assert_eq!(
                retrieved_path, &file_path,
                "Symbol should have correct path association"
            );
        }
...
"#;
        ...
        assert!(
            path_registry.path_to_index.contains_key(&file1),
            "PathRegistry should contain file1"
        );
        assert!(
            path_registry.path_to_index.contains_key(&file2),
            "PathRegistry should contain file2"
        );
        assert!(
            path_registry.path_to_index.contains_key(&file3),
            "PathRegistry should contain file3"
        );
        ...
        let file1_func = functions
            .iter()
            .find(|f| f.name == "file1_func")
            .expect("file1_func not found");
        let path_idx = file1_func.context.file_path_index;
        let retrieved_path = path_registry.get_path(path_idx);
        assert_eq!(
            retrieved_path, &file1,
            "file1_func should have correct path"
        );

        let file2_func = functions
            .iter()
            .find(|f| f.name == "file2_func")
            .expect("file2_func not found");
        let path_idx = file2_func.context.file_path_index;
        let retrieved_path = path_registry.get_path(path_idx);
        assert_eq!(
            retrieved_path, &file2,
            "file2_func should have correct path"
        );

        let file3_func = functions
            .iter()
            .find(|f| f.name == "file3_func")
            .expect("file3_func not found");
        let path_idx = file3_func.context.file_path_index;
        let retrieved_path = path_registry.get_path(path_idx);
        assert_eq!(
            retrieved_path, &file3,
            "file3_func should have correct path"
        );

        // Verify each class has correct path association
        let file1_class = classes
            .iter()
            .find(|c| c.name == "File1Class")
            .expect("File1Class not found");
        let path_idx = file1_class.context.file_path_index;
        let retrieved_path = path_registry.get_path(path_idx);
        assert_eq!(
            retrieved_path, &file1,
            "File1Class should have correct path"
        );

        let file2_class = classes
            .iter()
            .find(|c| c.name == "File2Class")
            .expect("File2Class not found");
        let path_idx = file2_class.context.file_path_index;
        let retrieved_path = path_registry.get_path(path_idx);
        assert_eq!(
            retrieved_path, &file2,
            "File2Class should have correct path"
        );
        ...
        assert_eq!(
            results.len(),
            1,
            "Should find exactly one match for file1_func"
        );
        let (symbol, _) = &results[0];
        let path_idx = symbol.context.file_path_index;
        let retrieved_path = path_registry.get_path(path_idx);
        assert_eq!(
            retrieved_path, &file1,
            "Search result should have correct path for file1_func"
        );
...
"#;
        ...
        let _multiple_decorated = functions
            .iter()
        ...
        let _decorated_method = functions
            .iter()
            .find(|f| {
                f.name == "decorated_method"
                    && !f.context.parent_context.is_empty()
                    && f.context
                        .parent_context
                        .iter()
                        .any(|p| p.name == "MultipleDecoratedClass")
            })
        ...
        assert!(
            func_count >= 5,
            "Should find at least 5 functions, found {}",
            func_count
        );
...
"#;
        ...
        let _double_decorated_method = functions
            .iter()
            .find(|f| {
                f.name == "double_decorated_method"
                    && !f.context.parent_context.is_empty()
                    && f.context
                        .parent_context
                        .iter()
        ...
        assert!(
            func_count >= 6,
            "Should find at least 6 functions, found {}",
            func_count
        );
        assert!(
            class_count >= 3,
            "Should find at least 3 classes, found {}",
            class_count
        );
        assert_eq!(syntax_errors, 0, "Should have no syntax errors");
        assert_eq!(io_errors, 0, "Should have no I/O errors");
