...
impl PathRegistry {
    ...
    pub fn total_path_bytes(&self) -> usize {
        ...
        let map_size =
            self.path_to_index.capacity() * (mem::size_of::<PathBuf>() + mem::size_of::<usize>());
        paths_size + map_size
    }
    ...
    pub fn debug_path_info(&self, file_path_index: usize) -> String {
        if file_path_index >= self.paths.len() {
            return format!(
                "ERROR: Index {} out of bounds (max index: {})",
                file_path_index,
            ...
            "function_definition" => {
                if let Some(name_node) = node.child_by_field_name("name") {
                    ...
                    let symbol_type = if current_parents.is_empty() {
                        SymbolType::Function
                    } else if current_parents
                        .last()
                        .is_some_and(|p| matches!(p.symbol_type, SymbolType::Class))
                    {
                        SymbolType::Method
                    } else {
...
pub fn save_symbols(path: &Path, stats: &SymbolStats) -> Result<()> {
    ...
    let symbol_data = SymbolData::new(functions_vec, classes_vec, path_registry.paths.clone());
...
mod tests {
    ...
    fn test_symbol_context_creation() {
        ...
        assert_eq!(
            *path_registry.get_path(context.file_path_index),
            PathBuf::from("/test/module/file.py")
        );
    }
    ...
    fn test_get_module_name() {
        ...
        assert_eq!(
            get_module_name(Path::new("relative/path/to/file.py")),
            "file"
        );
        assert_eq!(
            get_module_name(Path::new("just_filename.py")),
            "just_filename"
        );
    }
    ...
    fn test_get_fully_qualified_module() {
        ...
        assert_eq!(
            get_fully_qualified_module(
                Path::new("/base/dir/deeply/nested/module/file.py"),
                base_dir
            ),
