...
use symbol_experiments::symbols::{PathRegistry, Symbol, SymbolData, SymbolStats};
use tracing::info;
...
fn main() -> Result<()> {
    ...
    if let Some(symbols_file) = &args.symbols_file {
        ...
        print_symbols(
            &functions.iter().cloned().collect::<Vec<_>>(),
            &path_registry,
        );
        print_symbols(&classes.iter().cloned().collect::<Vec<_>>(), &path_registry);

        info!(
            "Found and listed {} functions and {} classes",
            functions.len(),
            classes.len()
        );
    }
