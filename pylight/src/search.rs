...
// Helper struct to group search-related parameters and reduce function arguments
...
impl std::str::FromStr for SearchAlgorithm {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            ...
            _ => Err(format!(
                "Unknown search algorithm: {}. Valid options are 'skim' or 'nucleo'",
                s
            )),
        }
    }
...
// Refactored helper function with reduced parameter count
fn process_collection_refactored(
    ...
    for symbol in symbols {
        ...
        if !seen_symbols.contains(&symbol_key) {
            ...
            let matches = context.pattern.match_list(&name_slice, matcher);
            ...
            if !matches.is_empty() && matches[0].1 > 0 {
                ...
                let query_lower = context.original_query.to_lowercase();
                ...
                if symbol_name_lower == query_lower {
                    ...
                    if context.debug {
                        println!(
                            "EXACT MATCH BOOSTED: {} (Score: {})",
                            symbol.name, score_i64
                        );
                    }
                }
                ...
                if context.debug {
                    ...
                    println!(
                        "{}: {} | Score: {} | File: {}:{} | Module: {} | Type: {:?} | Parents: {}",
                        symbol_type,
                        symbol.name,
                        score_i64,
                        path_registry
...
pub fn search_symbols(
    ...
    match algorithm {
        SearchAlgorithm::Skim => {
            ...
            let (results, metrics) =
                search_symbols_skim(query, functions, classes, path_registry, debug);
            (results, metrics)
        }
...
mod tests {
    ...
    fn test_search_algorithms() {
        ...
        for algorithm in [SearchAlgorithm::Skim, SearchAlgorithm::Nucleo] {
            ...
            assert!(
                !results.is_empty(),
                "Should find fuzzy matches with {:?}",
                algorithm
            );
            ...
            assert!(
                results.is_empty(),
                "Should not find any matches with {:?}",
                algorithm
            );
            ...
            assert!(
                !results.is_empty(),
                "Should find case-insensitive matches with {:?}",
                algorithm
            );
            println!(
                "{:?} metrics for case-insensitive match: {:?}",
                algorithm, metrics
            );
        }
    }
